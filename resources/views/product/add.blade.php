            ...
            <form action="{{ route('product.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="mb-6">
                    ...
                    <input class="w-full px-3 py-2 rounded-lg border-none" type="text" id="kode-produk"
                        placeholder="Kode Produk" name="code" value="{{ old('code') }}">
                ...
                {{--  <div class="mb-4">
                    ...
                    <input class="w-full px-3 py-2 rounded-lg border-none" type="text" id="jumlah-produk"
                        placeholder="Jumlah Produk" name="stock">
                </div>  --}}
                <div class="mb-4">
                    <label class="block text-white text-sm font-bold mb-2" for="jumlah-produk">JUMLAH PRODUK</label>
                    <div class="flex gap-2">
                        <input class="w-1/2 px-3 py-2 rounded-lg border-none text-gray-800" type="number"
                            id="stock" placeholder="Jumlah Produk" name="stock" value="{{ old('stock') }}">

                        <select name="stock_unit" class="w-1/2 px-3 py-2 rounded-lg border-none text-gray-800">
                            <option value="" disabled {{ old('stock_unit') ? '' : 'selected' }}>Pilih satuan
                            </option>
                            <option value="pcs" {{ old('stock_unit') == 'pcs' ? 'selected' : '' }}>pcs</option>
                            <option value="pack" {{ old('stock_unit') == 'pack' ? 'selected' : '' }}>pack</option>
                            <option value="dus" {{ old('stock_unit') == 'dus' ? 'selected' : '' }}>dus</option>
                            <option value="kg" {{ old('stock_unit') == 'kg' ? 'selected' : '' }}>kg</option>
                            <option value="porsi" {{ old('stock_unit') == 'porsi' ? 'selected' : '' }}>porsi</option>
                            <option value="kaleng" {{ old('stock_unit') == 'kaleng' ? 'selected' : '' }}>kaleng
                            </option>
                        </select>
                    </div>
                    @error('stock')
