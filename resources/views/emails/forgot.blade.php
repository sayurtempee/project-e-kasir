        ...
        <tr>
            <td align="center">
                ...
                <p style="word-break: break-word; font-size: 13px;">
                    <a
                        href="{{ url('reset-password/' . $token) }}?email={{ urlencode($email) }}">{{ url('reset-password/' . $token) }}?email={{ urlencode($email) }}</a>
                </p>
                <hr style="margin-top: 30px;">
                <p style="font-size: 13px; color: #999;">Jika Anda tidak meminta reset kata sandi, tidak perlu tindakan
                    lebih lanjut.</p>
                <p style="font-size: 13px; color: #999;">Terima kasih,<br>{{ config('app.name') }} Team</p>
            </td>
        </tr>
    </table>
</body>
